...
PairWFACCut::PairWFACCut(LAMMPS *lmp) : Pair(lmp)
{
  writedata = 1;
}
...
void PairWFACCut::compute(int eflag, int vflag)
{
  ...
  // Declare rmin and rminsq as 2D arrays with dimensions [n+1][n+1]
  //int n = atom->ntypes;
  //double rmin[n+1][n+1];
  //double rminsq[n+1][n+1];
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      jtype = type[j];
      //rmin = sigma[itype][jtype] * powint((2*nu[itype][jtype] + powint(sigma[itype][jtype]/cut[itype][jtype], 2*mu[itype][jtype]))/(1+2*nu[itype][jtype]),-1/(2*mu[itype][jtype]));
      //utils::logmesg(lmp,std::to_string(rmin[itype][jtype]));
      rminsq = pow(rmin[itype][jtype],2);
      
      //utils::logmesg(lmp,"rsq");
      //utils::logmesg(lmp,std::to_string(rsq));
      //utils::logmesg(lmp,"rminsq");
      //utils::logmesg(lmp,std::to_string(rminsq));
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        //if r is less than r* and epsilon is less than 0, flip the force value
        if (rsq <= rminsq) {
          if (epsilon[itype][jtype] < 0.0) {
            fpair = -factor_lj*e0nm[itype][jtype]*forcenm*powint(r2inv,mu[itype][jtype]+1);
          }
        }

        //if r is greater than r* and epsilon is less than 0, force is 0
        if (rsq > rminsq) {
          if (epsilon[itype][jtype] < 0.0) {
            fpair = 0.0;
          }
        ...
        if (eflag) {
          ...
          //if r is less than r* and epsilon is less than 0, flip the potential
          if (rsq <= rminsq) {
            if (epsilon[itype][jtype] < 0.0) {

              evdwl = -e0nm[itype][jtype] *
            (rm*powint(rn,2*nu[itype][jtype])) - eps[itype][jtype] - offset[itype][jtype];
            
            }
            
          }

          //if r is greater than r* and epsilon is less than 0, energy is 0
          if (rsq > rminsq) {
            if (epsilon[itype][jtype] < 0.0) {

              evdwl = 0.0 - offset[itype][jtype];
...
void PairWFACCut::settings(int narg, char **arg)
{
...
void PairWFACCut::coeff(int narg, char **arg)
{
  if (narg < 6 || narg > 7)
...
double PairWFACCut::init_one(int i, int j)
{
  ...
  e0nm[i][j] = epsilon[i][j]*2.0*nu[i][j]*powint(cut[i][j]/sigma[i][j],2*mu[i][j])
                       *powint((1+2.0*nu[i][j])/(2.0*nu[i][j])/(MathSpecial::powint(cut[i][j]/sigma[i][j],2*mu[i][j])-1.0),
                              2*nu[i][j]+1);
  eps[i][j] = epsilon[i][j];
  //rmin[i][j] = sigma[i][j] * powint((2*nu[i][j] + powint(sigma[i][j]/cut[i][j], 2*mu[i][j]))/(1+2*nu[i][j]),-1/(2*mu[i][j]));
  rmin[i][j] = sigma[i][j] * (pow((2*nu[i][j] + pow((sigma[i][j]/cut[i][j]),2*mu[i][j]))/(1+2*nu[i][j]),-1.0/(2.0*mu[i][j])));
...
void PairWFACCut::write_restart(FILE *fp)
{
...
void PairWFACCut::read_restart(FILE *fp)
{
...
void PairWFACCut::write_restart_settings(FILE *fp)
{
...
void PairWFACCut::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairWFACCut::write_data(FILE *fp)
{
...
void PairWFACCut::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
double PairWFACCut::single(int /*i*/, int /*j*/, int itype, int jtype,
                      double rsq, double /*factor_coul*/, double factor_lj,
                      double &fforce)
{
  double r2inv,rminv,rm,rn,forcenm,phinm,rminsq;
...
void *PairWFACCut::extract(const char *str, int &dim)
{
  dim = 2;
